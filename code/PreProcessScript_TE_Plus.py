...
def create_peak_peak_pairs(Peaks, only_peak_names_dict, max_distance=1000000):
    ...
    """
    peak_pairs = []
    # Group peaks by chromosome with start and end
    peaks_by_chr = {}
    for peak in Peaks:
        parts = peak.split('-')
        if len(parts) < 3:
    ...
    # For each chromosome, generate directed pairs
        ...
        for i in range(len(plist)):
            ...
            for j in range(len(plist)):
                ...
                if e1 < s2:
                    ...
                    dist = 0
                # include if no limit or within limit
...
# Transpose input matrix if not already prepared by modality_preprocess
transposed_matrix_path = coerce_output_path("cell_gene_trsps.parquet")
if not transposed_matrix_path.exists():
    load_and_transpose_matrix(sys.argv[3], transposed_matrix_path)

# Load gene names, peaks, and TF list
gene_names = load_file_to_list("gene_names")
Peaks = load_file_to_list("TE_peak_list.txt")
tf_list = load_file_to_list(f"GO_symbol_{sys.argv[1]}_regulation_of_transcription+sequence-specific_DNA_binding_list.txt")
...
# Map chromosome→list of genes for cis operations
...
if len(sys.argv) > 4:
    ...
    sif_filename = sys.argv[4]
    tf_gene_pairs, tf_peak_pairs, peak_gene_pairs = load_sif_connections(sif_filename, gene_names_dict)
    ...
    if mode == "1":
        ...
        # cis peak->peak pairs within default 1Mb
        gene_pairs_indices = create_peak_peak_pairs(Peaks, only_peak_names_dict, max_distance=None)
    else:
        print("Error: Invalid option for sys.argv[2].")
        sys.exit(1)

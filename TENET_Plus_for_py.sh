...
show_summary() {
    ...
    if [ "${STORE_LOCAL_TE_CHOICE,,}" = "on" ]; then
        echo "17) Local TE parameters  : chunk=$LOCAL_CHUNK_SIZE, buffer=$LOCAL_BUFFER_EDGES, export=$LOCAL_EXPORT_WORKERS, merge=$LOCAL_MERGE_WORKERS, read=$LOCAL_READ_BATCH, threads=$LOCAL_THREADS_FLAG, dtype=$LOCAL_VALUES_DTYPE"
    else
...
usage() {
    cat <<USAGE
Usage: $0 <input_file> <num_jobs> <trajectory_file> <cell_select_file> <history_k> <species> <mode_code> [modality] [screen_mode] [refine_method] [refine_topk] [refine_top_pct] [permute] [perm_n] [perm_topk] [perm_top_pct] [perm_fdr] [perm_q_alpha] [perm_alpha] [store_local_te] [time_stride] [time_pct] [time_seed]
...
if [ $# -eq 0 ]; then
    ...
    while true; do
        show_summary
        read -rp "Edit step number (1-18) or press Enter to continue: " EDIT_CHOICE
    ...
    cp -f "$0" "$OUTPUT_DIR/run_script_snapshot.sh" 2>/dev/null || true
    {
      echo "# Invocation:" 
      echo "$0 $*"
    } > "$OUTPUT_DIR/run_invocation.txt"
    echo "Final command: $0 $*"
...
if [[ "$SCREEN_MODE" == "linear" || "$SCREEN_MODE" == "poly" || "$SCREEN_MODE" == "gcmi" || "$SCREEN_MODE" == "disc" || "$SCREEN_MODE" == "ordinal" || "$SCREEN_MODE" == "kernel_grid" ]]; then
  ...
  if [[ "$PERMUTE" == "on" || "$PERMUTE" == "ON" || "$PERMUTE" == "1" ]]; then
    ...
    if ! run_with_stage "TE_DIRECT(${SCREEN_MODE})_PERMUTE" "$PYTHON" -m code.runTE_for_py_python_batch all_pairs.csv $2 $5 --enable_intermediate_save --mode "$SCREEN_MODE" "${PERM_ARGS[@]}" "${LOCAL_ARGS[@]}" "${TIME_ARGS[@]}"; then
      echo "Error in runTE_for_py_python_batch.py (direct permute). Exiting."
      exit 1
    fi
  else
    if ! run_with_stage "TE_DIRECT(${SCREEN_MODE})" "$PYTHON" -m code.runTE_for_py_python_batch all_pairs.csv $2 $5 --enable_intermediate_save --mode "$SCREEN_MODE" "${LOCAL_ARGS[@]}" "${TIME_ARGS[@]}"; then
